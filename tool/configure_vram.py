...
# -*- coding: utf-8 -*-
#
#  Auto VRAM Optimizer -- VRAM auto-config
#  "The Man, The Mythos, The Legend : KeilerHirsch"   (GNU GPL v3 or later)
#
#  Detects this PC's graphics-card memory and writes the mod's settings file,
#  so the in-game texture-streaming budget is set from real hardware with no
#  manual editing. Cross-vendor (NVIDIA / AMD / Intel) via the vram module.
#  This is the "true auto" half of Auto VRAM Optimizer: Lua cannot read physical
#  VRAM in-game, so the tool writes the value the mod then reads.
"""Detect VRAM and write the Auto VRAM Optimizer settings file."""
...
#: KEEP IN SYNC with the mod's settings filename in
#: FS25_AutoVRAMOptimizer/scripts/textureBudget.lua (settingsPath()).
MOD_NAME = "FS25_AutoVRAMOptimizer"
...
_HELP_TEXT = (
    "Auto-written by Auto VRAM Optimizer from your detected graphics-card memory. "
...
def settings_path(profile_dir: Path) -> Path:
    """Path of the mod's settings file inside a profile directory."""
    return profile_dir / "modSettings" / f"{MOD_NAME}.xml"


def write_settings(budget_gib: float, profile_dir: Path) -> Path:
    """Write the mod settings file and return its path.
...
def main() -> int:
    ...
    if not raw:
        ...
        print("  The mod will fall back to its built-in default, or you can set")
        print(f"  vramGiB by hand in <FS25 profile>/modSettings/{MOD_NAME}.xml")
        return 1
    ...
    print(f"\n  Wrote: {out}")
    print("  Install the mod and it will use this on the next game start.")
    return 0
